...
def otp_alert(sleep=10):
    ...
    def wait_and_alert():
        ...
        if otp:
            ...
            play_sound(os.path.join("resources", "refresh_OTP.mp3"))
...
def booking_alert(count=None):
    siren = Thread(target=play_sound, args=[os.path.join("resources", "Siren-SoundBible.com-1094437108.mp3")],
                   daemon=True)
    siren.start()
...
######## GLOBAL SETTINGS ##########
app = FastAPI(title="COWIN Booking")
otp = None
